...
@Component({
    ...
    templateUrl: './answer-delete-dialog.entity.component.html'
})
export class AnswerDeleteDialogEntityComponent {
    answer: IAnswer;
...
export class AnswerDeletePopupEntityComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ answer }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(AnswerDeleteDialogEntityComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
